...
public class ExprCodeGenerator extends Visitor<Value> {
	...
	public Value visitArrayIndex(ArrayIndex nd) {
		...
		Value idx = wrap(nd.getIndex().accept(this));
		
		return Jimple.v().newArrayRef(base, idx);
	}
	...
	public Value visitBinaryExpr(BinaryExpr nd) {
		...
		final Value lhs = wrap(nd.getLeft().accept(this)),
				rhs = wrap(nd.getRight().accept(this));
		
		Visitor<Value> binExprVisitor = new Visitor<Value>() {
			@Override
			...
			@Override
			//Take note it is visitModExpr not RemExpr from Visitor.java class
		...
		return nd.accept(binExprVisitor);
	}
